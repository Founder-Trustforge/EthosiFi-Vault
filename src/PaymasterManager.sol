...
contract PaymasterManager is IModule, ReentrancyGuard {
    ...
    event TransactionSponsored(
        address indexed account, bytes32 txHash, uint256 gasUsed, uint256 feePaid, address feeToken
    );
    ...
    event FreeQuotaExhausted(address indexed account, uint256 currentMonth);
    ...
    function onUninstall(bytes calldata) external override nonReentrant {
        ...
        address token = config.feeToken;
        uint256 balance = config.feeBalance;
        config.feeBalance = 0;
        delete configs[msg.sender];
        if (balance > 0 && token != address(0)) _safeTransfer(token, msg.sender, balance);
    }

    function validatePaymasterUserOp(address account, uint256 maxGasCost)
        external
        onlyEntryPoint
        nonReentrant
        returns (uint256)
    ...
    function depositFeeToken(address feeToken, uint256 amount) external nonReentrant {
        if (!feeTokens[feeToken].supported) revert TokenNotSupported();
    ...
    function withdrawFeeToken(uint256 amount) external nonReentrant {
        ...
        if (!config.initialized) revert NotInitialized();
        if (amount == 0) revert ZeroAmount();
        if (config.feeBalance < amount) revert InsufficientFeeBalance();
        address token = config.feeToken;
        config.feeBalance -= amount;
    ...
    function addFeeToken(address token, uint256 exchangeRate, string calldata symbol) external onlyOwner {
        ...
        if (exchangeRate == 0 || exchangeRate > MAX_EXCHANGE_RATE) revert InvalidExchangeRate();
    ...
    function updateExchangeRate(address token, uint256 newRate) external onlyOwner {
        if (!feeTokens[token].supported) revert TokenNotSupported();
        if (newRate == 0 || newRate > MAX_EXCHANGE_RATE) revert InvalidExchangeRate();
        feeTokens[token].exchangeRate = newRate;
    }

    function upgradeTier(address account, uint8 newTier) external onlyOwner {
        if (newTier > uint8(UserTier.ENTERPRISE)) revert InvalidTier();
        configs[account].tier = UserTier(newTier);
        emit TierUpgraded(account, UserTier(newTier));
    }

    function depositToPool(address feeToken) external payable onlyOwner {
    ...
    ///      Not exploitable by an attacker, but a genuine fund-loss bug: every dollar
    ///      deposited to sponsor gas fees was stuck forever. Added onlyOwner, CEI pattern,
    ///      and a balance check against the actual contract ETH balance to prevent
    ///      over-withdrawal.
    ...
    function _calculateFee(address feeToken, uint256 gasAmount) internal view returns (uint256) {
        ...
        uint256 totalGas = gasAmount + GAS_OVERHEAD;
        if (totalGas < gasAmount) revert FeeOverflow();
        uint256 fee = (totalGas * token.exchangeRate) / 1e18;
        if (fee > type(uint128).max) revert FeeOverflow();
        return fee;
    }
    ...
    function getConfig(address account)
        external
        view
        returns (UserTier tier, address feeToken, uint256 feeBalance, uint256 monthlyTxCount, uint256 lifetimeTxCount)
    {
    ...
    function getRemainingFreeQuota(address account) external view returns (uint256) {
        ...
        return FREE_MONTHLY_TX > c.monthlyTxCount ? FREE_MONTHLY_TX - c.monthlyTxCount : 0;
    }
    ...
    function preCheck(address, address, uint256, bytes calldata) external pure returns (bytes memory) {
        return "";
    }
