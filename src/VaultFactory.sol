...
contract VaultFactory is ReentrancyGuard {
    ...
    function createVault(
        ...
        if (guardianThreshold == 0 || guardianThreshold > _sumWeights(guardianWeights)) revert InvalidThreshold();
        ...
        bytes memory initData = abi.encodeWithSignature("initialize(address,address)", msg.sender, entryPoint);
        ...
        failedModules |= _installCoreValidators(
            vault, credentialId, pubKey, rpIdHash, guardianAddresses, guardianWeights, guardianThreshold
        );
    ...
    function computeVaultAddress(address creator, bytes32 salt) external view returns (address) {
        ...
        bytes32 hash = keccak256(abi.encodePacked(bytes1(0xff), address(this), finalSalt, bytecodeHash));
    ...
    function _installCoreValidators(
        ...
        if (
            !_installModule(
                vault, biometricValidator, TYPE_VALIDATOR, "BiometricValidator", abi.encode(credentialId, pubKey, rpIdHash)
            )
        ) failed |= 1 << 0;
        ...
        if (
            !_installModule(
                ...
                TYPE_VALIDATOR,
                "TimeLockValidator",
                abi.encode(guardians, threshold, uint256(0), emptyUSDC, "")
            )
        ) failed |= 1 << 1;

        if (
            !_installModule(
                vault, guardianValidator, TYPE_VALIDATOR, "GuardianValidator", _encodeGuardians(guardians, weights, threshold)
            )
        ) failed |= 1 << 2;

        if (
            !_installModule(
                vault, emergencyFreeze, TYPE_EXECUTOR, "EmergencyFreeze", abi.encode(guardians, threshold, uint256(0))
            )
        ) failed |= 1 << 3;

        if (
            !_installModule(
                vault, multiSigUpgradeGuard, TYPE_VALIDATOR, "MultiSigUpgradeGuard", abi.encode(guardians, threshold)
            )
        ) failed |= 1 << 4;
    }
    ...
    function _installProtectionModules(address vault) internal returns (uint256 failed) {
        ...
        if (!_installModule(vault, antiScamScreener, TYPE_HOOK, "AntiScamScreener", abi.encode(uint8(3), true))) {
            failed |= 1 << 6;
        }

        if (
            !_installModule(
                vault, plainEnglishExecutor, TYPE_EXECUTOR, "PlainEnglishExecutor", abi.encode(true, true, false)
            )
        ) failed |= 1 << 7;

        if (
            !_installModule(
                vault, aiThreatOracle, TYPE_HOOK, "AIThreatOracle", abi.encode(uint256(75), true, uint256(0))
            )
        ) failed |= 1 << 8;
        ...
        if (
            !_installModule(
                ...
                TYPE_HOOK,
                "DeepfakeGuard",
                abi.encode(false, uint256(70), new address[](0), uint256(1), uint256(0))
            )
        ) failed |= 1 << 9;
    }
    ...
    function _installUXModules(
        ...
        if (!_installModule(vault, paymasterManager, TYPE_EXECUTOR, "PaymasterManager", abi.encode(uint8(0), feeToken)))
        {
            failed |= 1 << 10;
        }
        ...
        if (seniorMode && seniorModeValidator != address(0)) {
            if (
                !_installModule(
                    ...
                    TYPE_VALIDATOR,
                    "SeniorModeValidator",
                    abi.encode(uint256(0), uint256(0), guardians, threshold, true)
                )
            ) failed |= 1 << 11;
        }
    }
    ...
    function _installModule(
        ...
        (success,) =
            vault.call(abi.encodeWithSignature("installModule(uint256,address,bytes)", moduleType, module, data));
    ...
    function _encodeGuardians(address[] calldata addrs, uint256[] calldata weights, uint256 threshold)
        internal
        pure
    ...
    function getVaultInfo(address vault)
        external
        view
        returns (address vaultOwner, uint256 deployedAt, bool seniorMode, uint256 modulesFailed)
    {
